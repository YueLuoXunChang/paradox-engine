...
## 二点五、发布面红线（⛔ 这些东西**不许写进仓库**）

> 本仓库是公开的：**写进去就等于发布**。曾经踩过的坑：私有工程名、内部文档编号、
> 个人邮箱进了文件与提交历史——只能靠改写历史清除（代价：所有 commit SHA 变化、
> 推送需 force）。所以宁可在写的时候就别写。

| ⛔ 不许写 | 该怎么做 |
|:--|:--|
| 本机绝对路径（盘符路径 / `/Users/…` / `/home/…`） | 相对路径，或"某盘/本机工作目录" |
| 真实姓名、邮箱、QQ、手机号 | 只留 GitHub 链接；提交身份用 GitHub noreply |
| 私有工程 / 内部档案的名字（别的仓、别的项目） | 通用表述："另一个项目""内部档案" |
| 内部文档**编号**（形如「两位数编号_名字.md」「两位数编号 蓝图」） | 通用表述："详规 / 总架构 / 蓝图"，**不编号** |
| 指向私有档案的章节引用（形如「某档案 §编号」） | 改写成"见 CHANGELOG" |
| 密钥 / 令牌 / 私钥 / 口令 | 永不入仓；用环境变量 |
| 医疗 / 法律 / 金融"建议"类示例 | 换中性题材（交通 / 供应链 / 质检 / 生态） |
| 抄来的代码或文字 | 标注来源，归借鉴区 |
| 门面版本号（"版本：vX.Y"） | 真发布才由作者打 tag |

**这些不是靠自觉，有测试守着**：`engine/test_public_face.py`（发布面抽查，回归必跑）、
`engine/test_docs_consistency.py`（门面数字必须等于实测）、`verify_install.py`（安装路径）。
本机如有额外私有名要拦，写进仓库根的 `.public-face-local.txt`（已在 .gitignore）。
