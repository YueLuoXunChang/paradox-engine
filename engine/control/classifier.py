...
==============================================================================
概念来源：落落体系规划文档（判类器/复杂度规格 + 总架构五步：
步 1 判类 / 步 2 判复杂度 / 步 3 切路 落码——内部规划，未随本仓库公开）。
本构件把"怎么认题、怎么决定走轻量道还是全量道"变成可跑规则。
...
TYPE_DEFS = {
    ...
    'T8': {'name': '证据冲突', 'weight': 3, 'signals': [
        # 语料回馈（2026-09-07）：原表只认「数据显示/报告说/
        # 相互矛盾的数据」这类固定短语——真实说法「两份实验报告结论相反」
        # 「研究说有效、另一份研究说无效」「数据互相矛盾」全部漏判为 T1/T2。
        # 现按真实用词扩表；平局（同分）时按类型序仍归 T2，不打乱纯冲突文本。
        '证据', '检测', '数据显示', '数据', '但表现', '阳性', '阴性', '实测',
        '报告说', '报告', '实验', '试验', '研究', '样本', '文献',
        '相互矛盾的数据', '互相冲突']},
    'T9': {'name': '学科题目', 'weight': 4, 'signals': [
        '建模', '机制', '如何', '原因', '演化', '多线', '通路',
        '系统', '理论', '模型', '假设机制']},
    'T10': {'name': '定义辨析', 'weight': 2, 'signals': [
        '区别', '不同', '是不是一回事', '边界', '定义', '辨析',
        '什么算', '算不算']},
    'T11': {'name': '假设检验', 'weight': 2, 'signals': [
        # 语料回馈（2026-09-07，第二处闭环）：原表只有"靠不靠谱/有效率/显著"
        # 这类评价词，真实说法"这个调查说八成的人支持，样本只有一百人"里
        # 「调查/样本/比例」全不在表中，于是被判成 T3（命中了"支持"）。
        # 现补抽样/调查/比例类用词。
        '有效率', '可能性', '概率', '信不信', '靠不靠谱', '要不要信',
        '可靠', '置信', '显著', '样本', '调查', '抽样', '受访', '统计',
        '比例', '百分比', '多少人']},
}
...
def _classify(text):
    """题型计分（详规 §1.1：命中信号 += weight 一次/信号去重）。"""
    hits = []
...
def _complexity(text, hits):
    ...
    # 题型修正（总架构 §4.1 落码）：T9 学科题目"罕见 L1"——术语密集即建模
    ...
    if 'T9' in tids and t9_hits >= 4:
        level = 'L3'
        flags.append('T9学科密集→L3（总架构 §4.1 规则）')
    elif 'T9' in tids and t9_hits == 3:
        if level == 'L1':
            level = 'L2'
        flags.append('T9学科信号3→L2（总架构 §4.1 规则）')
...
if __name__ == '__main__':
    ...
    # 5) 学科题目（T9-L3）
    r5 = run({'text': '城市交通系统为何越修越堵：通勤者改走新路使商业区车流集中，公交分担率下滑又推高私家车出行，能否建模其多线演化路径、机制如何，以及「扩容反而更堵」算不算悖论？'})
    assert r5['main_type'] == 'T9', r5
    assert r5['level'] == 'L3', r5
    print(f"✅ 拥堵演化 → {r5['main_type']} {r5['level']} → "
          f"{r5['route']['pipeline']}")
