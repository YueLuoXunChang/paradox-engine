...
# ── 用例4：走查二——拥堵演化（T9-L3 全量道）
r = run({'text': '城市交通系统为何越修越堵：通勤者改走新路使商业区车流集中，公交分担率下滑又推高私家车出行，能否建模其多线演化路径、机制如何，以及「扩容反而更堵」算不算悖论？'})
check("拥堵演化 → T9", r['main_type'] == 'T9', str(r))
check("拥堵演化 → L3", r['level'] == 'L3', str(r))
